...
from app.db.crud import get_animal_by_id, create_animal, delete_animal, get_all_animals, get_breed_by_id, update
from app.models import Animal
from app.core.deps import get_current_user, require_animal

router = APIRouter(prefix='/animals', tags=["animals"], dependencies=[Depends(get_current_user)])

@router.post("", response_model=AnimalResponse)
def create_animal_endpoint(animal_data: AnimalCreate, session: Session = Depends(get_db)):
    """breed_id и parent_id проверяются заранее, чтобы 404 (нет такой ссылки)
    не терялось за общим 409 от FK-нарушения."""
...
def get_animals(session: Session = Depends(get_db)):
    return get_all_animals(session=session)

@router.get("/{animal_id}", response_model=AnimalResponse)
def get_animal(animal_id: int, animal: Animal = Depends(require_animal)):
    return animal
...
def update_animal_endpoint(
        ...
        animal: Animal = Depends(require_animal)
):
    update_data = data.model_dump(exclude_unset=True)

    # проверяем ссылки только если их реально прислали в патче
    ...
    try:
        return update(session=session, obj=animal, data=update_data)
    except IntegrityError:
        session.rollback()
        raise HTTPException(status_code=409, detail='Data conflict: check inventory number, breed and parent references')
...
def delete_animal_endpoint(
    ...
    # IntegrityError - на это животное ссылается хотя бы одна запись взвешивания
    try:
        delete_animal(session=session, animal=animal)
    except IntegrityError:
        session.rollback()
        raise HTTPException(status_code=409, detail='Cannot delete: animal is referenced by existing weighting records')
